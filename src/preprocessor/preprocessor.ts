export function preprocess(code: string, defines: Record<string, string> = {}, checks: Record<string, boolean> = {}) {
    ...
    for (const line of lines) {
        ...
        if (match !== null) {
            if (match.groups != null) {
                const nextIgnore = !(checks[match.groups["condition"].trim()] === true);
    ...
    if (matches != null) {
        for (let match of matches) {
            if (match.groups != null) {
                ...
                const defineValue = defines[defineName] ?? match.groups["default"];

                if (defineValue == null) {
                    throw new Error(`no define value or default value for '${defineName}'`);
                }
